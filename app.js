...
function seedInjuryRow(raw, abbr, prior) {
  ...
  // win over a stale localStorage manual (Darnold 1.5 / Murray 0.5).
  if (seedManual) {
    impact = seedImpact;
...
async function loadNfl() {
  ...
  const scaleReq = fetch("./data/injury-scale.json?v=injcap21b");
  const allProReq = fetch("./data/allpro-last3.json?v=injcap21b");
  const injReq = fetch("./data/injury-2026.json?v=injcap21b");
...
function pillarWeights() {
  const w = (priorData && priorData.weights) || { off: 0.3875, def: 0.3875, st: 0.025, take: 0.075, give: 0.125 };
  return w;
}
