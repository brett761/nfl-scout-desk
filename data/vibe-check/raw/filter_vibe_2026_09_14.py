#!/usr/bin/env python3
"""Filter 2026-09-14 vibe-check raw news into a candidate keep set.
...
VIBE = HERE.parent
RAW_FILES = [
    HERE / f"{DATE}-batch1.json",
    HERE / f"{DATE}-batch2.json",
    HERE / f"{DATE}-batch3.json",
    HERE / f"{DATE}-batch4.json",
    # legacy division names if a puller used them
...
def merge_raw_clubs(raw_paths: list[Path]) -> tuple[list[dict], list[str], list[str]]:
    ...
    for path in raw_paths:
        ...
        elif isinstance(data, dict) and isinstance(data.get("clubs"), dict):
            for abbr, c in data["clubs"].items():
